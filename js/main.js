...
function update(dt) {
    ...
    if (currentState === 'MATCH_END' && tournamentMode && tournamentPendingMatch) {
        ...
        const playerLost = playerIsA ? (teamAScore < teamBScore) : (teamBScore < teamAScore);

        if (!window._endFlow || window._endFlow.uid !== pending.uid) {
            const offer = playerLost && typeof AdManager !== 'undefined' &&
                AdManager.canOfferTournamentContinue(pending.uid, isFinal, isGroup);
            window._endFlow = { uid: pending.uid, t: 0, offer: !!offer, duration: offer ? AdManager.getOfferSeconds() : 0 };
        }
        const flow = window._endFlow;
        const adBusy = typeof AdManager !== 'undefined' && AdManager.isAdRequestInFlight();
        if (!adBusy) flow.t += dt;
        if (flow.t < (flow.offer ? flow.duration + 0.7 : 1.5)) return;
        ...
        const plOutcome = plScore > opScore ? 'win' : (plScore === opScore ? 'draw' : 'loss');
        ...
        // "Rounds survived" for XP: group stage matches count as round 0
        // survived on a win; knockout rounds use (roundIdx + 1) so reaching
        // and winning the final (roundIdx 3) survives 4 rounds. Champion
        // status is read from TournamentManager.champion, which
        // recordPlayerMatchResult() (called just above) has already set if
        // this was the winning final.
...
function syncGameplayPing() {
    const adBusy = typeof AdManager !== 'undefined' && AdManager.isAdRequestInFlight();
    const active = currentState === 'PLAY' && matchState === 'PLAY' && !adBusy;
