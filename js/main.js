...
function update(dt) {
    ...
    if (currentState === 'MATCH_END' && tournamentMode && tournamentPendingMatch) {
        ...
        // ===== AD STRATEGY: knockout loss -> hold off recording the result =====
        // A knockout loss is eligible for the "watch an ad to continue"
        // offer (never for group-stage matches, the final, or 1v1 — see
        // AdManager.canOfferTournamentContinue). If eligible, the result
        // is NOT committed to TournamentManager yet: committing a
        // knockout loss immediately simulates/advances the rest of the
        // bracket (see recordPlayerMatchResult -> completeKnockoutRoundAfterPlayer
        // -> runRemainingTournament in tournament.js), which cannot be
        // safely undone without touching tournament calculation internals
        // — exactly what this task must not rewrite. So instead: show the
        // offer BEFORE recording anything. "Continue without ad" or an ad
        // failure records the real result exactly as before; watching the
        // ad replays this same fixture with bonus time instead of ever
        // recording a loss for it.
        const isKnockoutRound = !isGroup;
        // Group-stage losses are intentionally never eligible here — the
        // spec's rewarded "continue" is a per-match knockout mechanic
        // ("non-final tournament match"), not a group-stage save. A group
        // exit is decided by overall standings across multiple matches,
        // not a single loss, so one rewarded ad "undoing" it would let a
        // single ad reverse a whole stage's worth of results instead of
        // one match — a materially different (and much stronger) reward
        ...
        const playerLostThisMatch = (playerTeamId === teamAId) ? (teamAScore < teamBScore) : (teamBScore < teamAScore);
        const offerEligible = isKnockoutRound && playerLostThisMatch && !isFinalRound
            && typeof AdManager !== 'undefined'
            && AdManager.canOfferTournamentContinue(pending.uid, false);

        if (offerEligible) {
            window._pendingContinueOffer = {
                uid: pending.uid, teamA: pending.teamA, teamB: pending.teamB,
                roundIndex: roundIdx, teamAScore, teamBScore
            };
    ...
    if (currentState === 'TOURNAMENT_CONTINUE_OFFER') return;
...
function bootstrap() {
    ...
    try {
        ...
        if (typeof PlatformSDK !== 'undefined') {
            PlatformSDK.init().catch(e => console.error('[ProStriker] PlatformSDK init failed:', e));
        }
