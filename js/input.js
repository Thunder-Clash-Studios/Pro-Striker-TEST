...
function exitMatchEndToMenu() {
    if (typeof AdManager === 'undefined' || typeof PlatformSDK === 'undefined' || AdManager.isAdRequestInFlight()) {
        currentState = 'MENU';
        updateTouchUI();
        return;
    }
    if (AdManager.shouldOfferNormalMatchAd()) {
        AdManager.requestNormalMatchAd(() => {
            currentState = 'MENU';
            updateTouchUI();
        });
    } else {
...
function requestTournamentContinueAdAndResume() {
    const offer = window._pendingContinueOffer;
    if (!offer || typeof AdManager === 'undefined') { declineTournamentContinueOffer(); return; }

    AdManager.requestTournamentContinueAd(offer.uid, {
        onReward: () => {
            window._pendingContinueOffer = null;
            // Replay the SAME fixture (identical teams/uid/round) with a
            // comeback bonus added to the clock. This deliberately reuses
            // startTournamentMatch()'s exact team/side-assignment logic
            // (red = player) rather than duplicating it, so the comeback
            // match is indistinguishable in setup from the original.
            const bonus = AdManager.getBonusSeconds();
            const replayMatch = {
                uid: offer.uid, type: 'knockout', roundIndex: offer.roundIndex,
                teamA: offer.teamA, teamB: offer.teamB
            };
            startTournamentMatch(replayMatch);
            // Bonus is added on top of the half length the match already
            // started with, as extra match clock — a straightforward
            // "comeback opportunity" rather than a rewritten match format.
            matchClock += bonus;
            SoundManager.playSFX('confirm');
        },
        onDeclinedOrFailed: () => {
