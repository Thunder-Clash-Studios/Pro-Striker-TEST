...
1. **Change this configuration:** in `js/platform/platformConfig.js`, find
   the `gamemonetize` block inside `PLATFORM_SETTINGS` and set your real
   Game ID.
2. **Set this GameId:**
   ```js
   gamemonetize: {
       gameId: 'PASTE_YOUR_REAL_GAMEMONETIZE_ID_HERE'
   }
   ```
   Get this hash from your GameMonetize.com "My games" dashboard.
3. **SDK configuration used:** `js/platform/adapters/gameMonetizeAdapter.js`
   automatically injects the official `window.SDK_OPTIONS` snippet and loads
   `https://api.gamemonetize.com/sdk.js` using the Game ID you set in step 2.
   You do not need to touch `index.html` or the adapter file.
4. **Upload/build like this:** zip the `Pro Striker/` folder (root must
   contain `index.html`) and upload via GameMonetize's Game Management > My
   games > [your game] > drop file to upload. Click "Verify Game", then
   "Request Activation" once verification passes.
